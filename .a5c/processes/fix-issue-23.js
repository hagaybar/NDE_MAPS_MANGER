/**
 * @process fix-issue-23
 * @description Implement the fix for issue #23 (RTL panel positioning)
 *              following docs/superpowers/plans/2026-05-12-issue-23-rtl-panel-positioning.md
...
function buildAgentTask(taskNumber, taskTitle) {
  return defineTask(`issue-23-task-${taskNumber}`, (args, taskCtx) => ({
    kind: 'agent',
    title: `Issue #23 Task ${taskNumber}: ${taskTitle}`,
    agent: {
...
const taskOne = buildAgentTask(1, 'Write failing Playwright positioning spec');
const taskTwo = buildAgentTask(2, 'Apply CSS fix; re-run; verify e2e suite');
const taskThree = buildAgentTask(3, 'Push branch and open PR (no merge)');

export async function process(inputs, ctx) {
  const { projectRoot, planPath, repoSlug } = inputs;
  ctx.log('info', 'Starting issue #23 fix', { projectRoot, planPath, repoSlug });
  const args = { projectRoot, planPath, repoSlug };

  const tasks = [[0, taskZero], [1, taskOne], [2, taskTwo], [3, taskThree]];
  const summaries = {};
  let prUrl = null;
