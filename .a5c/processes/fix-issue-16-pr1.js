/**
 * @process fix-issue-16-pr1
 * @description Implement issue #16 PR 1 (map-object data typing foundation)
 *              per docs/superpowers/plans/2026-05-12-issue-16-pr1-map-object-data-typing.md
...
function buildAgentTask(taskNumber, taskTitle) {
  return defineTask(`issue-16-pr1-task-${taskNumber}`, (args, taskCtx) => ({
    kind: 'agent',
    title: `Issue #16 PR 1 Task ${taskNumber}: ${taskTitle}`,
    agent: {
...
const taskOne = buildAgentTask(1, 'Write failing CSV ↔ marker alignment regression-guard test');
const taskTwo = buildAgentTask(2, 'Write and run the migration script');
const taskThree = buildAgentTask(3, 'Write failing unit tests for location-model.js');
const taskFour = buildAgentTask(4, 'Implement location-model.js');
const taskFive = buildAgentTask(5, 'Refactor map-editor.js to use indexShelfLocations');
const taskSix = buildAgentTask(6, 'Run full e2e suite — verify no regression');
const taskSeven = buildAgentTask(7, 'Push branch and open PR (no merge)');

export async function process(inputs, ctx) {
  const { projectRoot, planPath, repoSlug } = inputs;
  ctx.log('info', 'Starting issue #16 PR 1', { projectRoot, planPath, repoSlug });
