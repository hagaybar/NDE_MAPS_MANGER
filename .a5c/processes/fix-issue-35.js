/**
 * @process fix-issue-35
 * @description Implement issue #35 (Map Files download + replace flow)
 *              per docs/superpowers/plans/2026-05-13-issue-35-map-download-replace.md
...
function buildAgentTask(taskNumber, taskTitle) {
  return defineTask(`issue-35-task-${taskNumber}`, (args, taskCtx) => ({
    kind: 'agent',
    title: `Issue #35 Task ${taskNumber}: ${taskTitle}`,
    agent: {
...
const taskOne = buildAgentTask(1, 'Write failing Jest unit tests for svg-manager');
const taskTwo = buildAgentTask(2, 'Add FALLBACKS keys + replaceFile helper + __test export');
const taskThree = buildAgentTask(3, 'Add Download + Replace buttons + delegated click handlers');
const taskFour = buildAgentTask(4, 'Write Playwright e2e for download + replace + role-gating');
const taskFive = buildAgentTask(5, 'Run the full test suites — verify no regression');
const taskSix = buildAgentTask(6, 'Push the branch and open the PR');

export async function process(inputs, ctx) {
  const { projectRoot, planPath, repoSlug } = inputs;
  ctx.log('info', 'Starting issue #35', { projectRoot, planPath, repoSlug });
  const args = { projectRoot, planPath, repoSlug };

  const tasks = [
    [0, taskZero], [1, taskOne], [2, taskTwo], [3, taskThree],
    [4, taskFour], [5, taskFive], [6, taskSix],
  ];
  const summaries = {};
  let prUrl = null;
