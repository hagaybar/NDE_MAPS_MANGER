...
// -----------------------------------------------------------------------------
test('Map Editor: select shelf opens the side panel', async ({ page }) => {
...
test('Map Editor: warning shown on overlap, save still allowed', async ({ page }) => {
  ...
  await expect(page.locator('.map-panel__warn-banner')).toBeVisible();
...
test('Map Editor: reassign via map-pick', async ({ page }) => {
  ...
  await clickShelf(page, 'A1');
  // Panel should not still mention the previous Soc 100-110 range.
  const panel = page.locator('#map-side-panel');
  await expect(panel).not.toContainText('100-110');
});
...
test.describe('Map Editor as editor', () => {
  ...
  test('locked shelves are hatched and unclickable when fully locked', async ({ page }) => {
    ...
    // The panel stays in idle mode (single-shelf select on a fully-locked shelf
    // is a no-op per the spec / svg-interaction.js — no shelf card opens).
    await expect(page.locator('#map-side-panel .map-panel--idle')).toBeVisible();
    await expect(page.locator('#map-side-panel .map-card')).toHaveCount(0);
  });
});
